...
import { MyService, MyServiceCosting, MyServiceCostingUpdate, MyServiceCostingCreate } from '../api'
...
export default function MyServices() {
    ...
    const handleOpen = async (service) => {
        const api = await MyServiceCosting(service, 'installation')
        switch (api.status) {
            case 200:
                const apiRes = await api.json()
    ...
    const handleChange = async (index, catagory) => {
        const api = await MyServiceCosting(costing.service, catagory)
        switch (api.status) {
            case 200:
                const apiRes = await api.json()
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const api = await MyServiceCostingUpdate(costing)
        switch (api.status) {
            case 200:
                const apiRes = await api.json()
                ...
                break;
            case 404:
                const data = await MyServiceCostingCreate(costing)
                                    ...
                                    <Tab.Panels>
                                        {details.catargories?.map((data, index) => {
                                            return <Tab.Panel key={index} className='capitalize space-y-4p'>
                                                <p>cost for {costing.catagory} of {costing.service}</p>.
