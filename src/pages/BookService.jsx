...
import { bookPriceDetail, Book } from '../api'
...
export default function BookService() {
  ...
  const form = useFormik({
    initialValues: {
      service_provider: searchParams.get('name'),
    ...
    onSubmit: async (values) => {
      const api = await Book(form.values.service_provider, form.values.service, form.values.catagory, form.values)
      switch (api.status) {
        case 201:
            storeState.setmessage({
                hidden: false,
                message: `Service Successfull for ${form.values.service} ${form.values.catagory}`
            })
            navigate('/dashboard')
          break;
        default:
  ...
  const loadPriceData = useCallback(async () => {
    if (isAuthenciated) {
      const api = await bookPriceDetail(form.values.service_provider, form.values.service, form.values.catagory)
      switch (api.status) {
        case 200:
  ...
  }, [navigate, form.values.service_provider, form.values.service, form.values.catagory , isAuthenciated])
    ...
    <Protected>
      ...
      <main className='min-h-screen p-4 space-y-4 w-full lg:w-[65%] mx-auto mt-20'>
        ...
        <div className='border border-violet-600 bg-violet-100 dark:bg-violet-500/10 rounded p-4'>
          You are booking "{form.values.service_provider}" for "{form.values.catagory}" of your "{form.values.service}"
        </div>
        <form className='space-y-4' onSubmit={form.handleSubmit}>
          ...
          />
          <p className='flex items-center'>Total amount <CurrencyRupeeIcon className='w-5 h-5 mx-1' /> {price} for {form.values.service} {form.values.catagory}</p>
...
export { alterphonenumber, address }